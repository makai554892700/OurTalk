...
//                , new InetSocketAddress("85.174.227.52", 59280)), false));
    ...
    public static boolean proxyCheckProxy(String host, Integer port) {
        ...
        if (PROXY_IP_BASE.equals(proxyIP)) {
            LogUtils.log("proxy error.proxyIP=" + proxyIP + ";currentIP=" + PROXY_IP_BASE + ";host=" + host + ";port=" + port);
            return false;
        }
        if (times++ % 30 == 0) {
            LogUtils.log("proxy error.proxyIP=" + proxyIP + ";currentIP=" + PROXY_IP_BASE + ";host=" + host + ";port=" + port);
    ...
    public static String proxyProxyIP(String host, Integer port) {
        ...
        HttpUtils.getInstance().getURLResponse(String.format(URL_RPOXY_IP, host, port)
    ...
    public static boolean checkProxy(Proxy proxy, boolean isHttps, int timeOut) {
        ...
        if (!result) {
            LogUtils.log("proxy error.proxyIP=" + proxyIP + ";currentIP=" + currentIP + ";proxy=" + proxy.address());
        }
        LogUtils.log("proxy error.proxyIP=" + proxyIP + ";currentIP=" + currentIP + ";proxy=" + proxy.address());
        return result;
    }
    ...
    public static String getProxyIP(Proxy proxy, boolean isHttps, int timeOut) {
        final StringBuilder result = new StringBuilder();
        HttpUtils.getInstance().getURLResponse(isHttps ? URL_GET_HTTPS_IP : URL_GET_HTTP_IP
    ...
    public static int ip2Key(String ip) {
        ...
        if (ip != null && !ip.isEmpty()) {
            ...
            if (parts.length == 4) {
                ...
                LogUtils.log("parts.length=" + parts.length + ";ip=" + ip);
            }
        }
        return result;
    }
